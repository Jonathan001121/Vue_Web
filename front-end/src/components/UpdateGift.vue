<template>
  <div>
    ...
    <form class="container">
      ...
      <button class="btn btn-primary" type="submit" @click="updateGift()">
        Update
      </button>
      <button class="btn btn-danger" type="button" @click="deleteGift()">
        Delete
      </button>
    </form>
  </div>
...
export default {
  ...
  setup() {
    ...
    onMounted(async () => {
      var response = await fetch("/api/gift/detail/" + route.params.id);
      if (response.ok) {
        gift.value = await response.json();
